.class public final enum Lcom/smartadserver/android/library/model/SASAdStatus;
.super Ljava/lang/Enum;
.source ""


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "Ljava/lang/Enum<",
        "Lcom/smartadserver/android/library/model/SASAdStatus;",
        ">;"
    }
.end annotation


# static fields
.field private static final synthetic $VALUES:[Lcom/smartadserver/android/library/model/SASAdStatus;

.field public static final enum EXPIRED:Lcom/smartadserver/android/library/model/SASAdStatus;

.field public static final enum LOADING:Lcom/smartadserver/android/library/model/SASAdStatus;

.field public static final enum NOT_AVAILABLE:Lcom/smartadserver/android/library/model/SASAdStatus;

.field public static final enum READY:Lcom/smartadserver/android/library/model/SASAdStatus;

.field public static final enum SHOWING:Lcom/smartadserver/android/library/model/SASAdStatus;


# direct methods
.method public static constructor <clinit>()V
    .locals 12

    const/4 v11, 0x7

    new-instance v0, Lcom/smartadserver/android/library/model/SASAdStatus;

    const-string v1, "AOsALNLABTI_E"

    const-string v1, "NOT_AVAILABLE"

    const/4 v11, 0x0

    const/4 v2, 0x0

    const/4 v11, 0x5

    invoke-direct {v0, v1, v2}, Lcom/smartadserver/android/library/model/SASAdStatus;-><init>(Ljava/lang/String;I)V

    const/4 v11, 0x3

    sput-object v0, Lcom/smartadserver/android/library/model/SASAdStatus;->NOT_AVAILABLE:Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x1

    new-instance v1, Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x7

    const-string v3, "OGDmLAN"

    const-string v3, "LOADING"

    const/4 v11, 0x2

    const/4 v4, 0x1

    const/4 v11, 0x7

    invoke-direct {v1, v3, v4}, Lcom/smartadserver/android/library/model/SASAdStatus;-><init>(Ljava/lang/String;I)V

    const/4 v11, 0x7

    sput-object v1, Lcom/smartadserver/android/library/model/SASAdStatus;->LOADING:Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x3

    new-instance v3, Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x5

    const-string v5, "DYRAo"

    const-string v5, "READY"

    const/4 v11, 0x2

    const/4 v6, 0x2

    const/4 v11, 0x5

    invoke-direct {v3, v5, v6}, Lcom/smartadserver/android/library/model/SASAdStatus;-><init>(Ljava/lang/String;I)V

    const/4 v11, 0x3

    sput-object v3, Lcom/smartadserver/android/library/model/SASAdStatus;->READY:Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x2

    new-instance v5, Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x7

    const-string v7, "INOWHbS"

    const-string v7, "SHOWING"

    const/4 v11, 0x4

    const/4 v8, 0x3

    const/4 v11, 0x6

    invoke-direct {v5, v7, v8}, Lcom/smartadserver/android/library/model/SASAdStatus;-><init>(Ljava/lang/String;I)V

    sput-object v5, Lcom/smartadserver/android/library/model/SASAdStatus;->SHOWING:Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x7

    new-instance v7, Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x6

    const-string v9, "RDEXIEu"

    const-string v9, "EXPIRED"

    const/4 v11, 0x2

    const/4 v10, 0x4

    const/4 v11, 0x1

    invoke-direct {v7, v9, v10}, Lcom/smartadserver/android/library/model/SASAdStatus;-><init>(Ljava/lang/String;I)V

    const/4 v11, 0x0

    sput-object v7, Lcom/smartadserver/android/library/model/SASAdStatus;->EXPIRED:Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x7

    const/4 v9, 0x5

    const/4 v11, 0x3

    new-array v9, v9, [Lcom/smartadserver/android/library/model/SASAdStatus;

    aput-object v0, v9, v2

    const/4 v11, 0x6

    aput-object v1, v9, v4

    aput-object v3, v9, v6

    aput-object v5, v9, v8

    const/4 v11, 0x1

    aput-object v7, v9, v10

    const/4 v11, 0x4

    sput-object v9, Lcom/smartadserver/android/library/model/SASAdStatus;->$VALUES:[Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v11, 0x6

    return-void
.end method

.method private constructor <init>(Ljava/lang/String;I)V
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()V"
        }
    .end annotation

    const/4 v0, 0x6

    invoke-direct {p0, p1, p2}, Ljava/lang/Enum;-><init>(Ljava/lang/String;I)V

    const/4 v0, 0x6

    return-void
.end method

.method public static valueOf(Ljava/lang/String;)Lcom/smartadserver/android/library/model/SASAdStatus;
    .locals 2

    const-string v1, "  ~@~@~@~@~@~@~@~@~@~@~   Smob - Mod obfuscation tool v1.4 by Kirlif\'   ~@~@~@~@~@~@~@~@~@~@~  "

    const-class v0, Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v1, 0x3

    invoke-static {v0, p0}, Ljava/lang/Enum;->valueOf(Ljava/lang/Class;Ljava/lang/String;)Ljava/lang/Enum;

    move-result-object p0

    const/4 v1, 0x6

    check-cast p0, Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v1, 0x6

    return-object p0
.end method

.method public static values()[Lcom/smartadserver/android/library/model/SASAdStatus;
    .locals 2

    const/4 v1, 0x0

    sget-object v0, Lcom/smartadserver/android/library/model/SASAdStatus;->$VALUES:[Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v1, 0x2

    invoke-virtual {v0}, [Lcom/smartadserver/android/library/model/SASAdStatus;->clone()Ljava/lang/Object;

    move-result-object v0

    const/4 v1, 0x7

    check-cast v0, [Lcom/smartadserver/android/library/model/SASAdStatus;

    const/4 v1, 0x1

    return-object v0
.end method
