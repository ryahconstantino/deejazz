.class public Lz20;
.super Ljava/lang/Object;
.source ""

# interfaces
.implements Lq20;


# static fields
.field public static final b:Ljava/lang/String;


# instance fields
.field public final a:Lo30;


# direct methods
.method public static constructor <clinit>()V
    .locals 2

    const/4 v1, 0x7

    const-class v0, Lz20;

    const-class v0, Lz20;

    invoke-static {v0}, Lcom/braze/support/BrazeLogger;->getBrazeLogTag(Ljava/lang/Class;)Ljava/lang/String;

    move-result-object v0

    const/4 v1, 0x4

    sput-object v0, Lz20;->b:Ljava/lang/String;

    const/4 v1, 0x3

    return-void
.end method

.method public constructor <init>(Lo30;)V
    .locals 1

    const/4 v0, 0x6

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/4 v0, 0x5

    iput-object p1, p0, Lz20;->a:Lo30;

    const/4 v0, 0x3

    return-void
.end method


# virtual methods
.method public a(Landroid/app/Activity;Lcom/braze/models/inappmessage/IInAppMessage;)Landroid/view/View;
    .locals 10
    .annotation build Landroid/annotation/SuppressLint;
        value = {
            "AddJavascriptInterface"
        }
    .end annotation

    const-string v9, "  ~@~@~@~@~@~@~@~@~@~@~   Smob - Mod obfuscation tool v1.4 by Kirlif\'   ~@~@~@~@~@~@~@~@~@~@~  "

    invoke-virtual {p1}, Landroid/app/Activity;->getLayoutInflater()Landroid/view/LayoutInflater;

    move-result-object v0

    const/4 v9, 0x7

    sget v1, Lcom/appboy/ui/R$layout;->com_braze_inappmessage_html_full:I

    const/4 v9, 0x6

    const/4 v2, 0x0

    const/4 v9, 0x3

    invoke-virtual {v0, v1, v2}, Landroid/view/LayoutInflater;->inflate(ILandroid/view/ViewGroup;)Landroid/view/View;

    move-result-object v0

    const/4 v9, 0x3

    check-cast v0, Lcom/braze/ui/inappmessage/views/InAppMessageHtmlFullView;

    const/4 v9, 0x7

    new-instance v1, Lcom/braze/configuration/BrazeConfigurationProvider;

    const/4 v9, 0x5

    invoke-virtual {p1}, Landroid/app/Activity;->getApplicationContext()Landroid/content/Context;

    move-result-object v3

    const/4 v9, 0x1

    invoke-direct {v1, v3}, Lcom/braze/configuration/BrazeConfigurationProvider;-><init>(Landroid/content/Context;)V

    const/4 v9, 0x6

    invoke-virtual {v1}, Lcom/braze/configuration/BrazeConfigurationProvider;->getIsTouchModeRequiredForHtmlInAppMessages()Z

    move-result v1

    const/4 v9, 0x4

    if-eqz v1, :cond_0

    const/4 v9, 0x7

    invoke-static {v0}, Ll40;->isDeviceNotInTouchMode(Landroid/view/View;)Z

    move-result v1

    const/4 v9, 0x0

    if-eqz v1, :cond_0

    const/4 v9, 0x6

    sget-object p1, Lz20;->b:Ljava/lang/String;

    const/4 v9, 0x4

    const-string p2, ".nseiuee anl  oeopi hrlssstc ui  crstTv  aiihotsunorouiey dsroymgeeTcc e r pdre prmeiteyh eoc nihdtlna.qutsr"

    const-string p2, "The device is not currently in touch mode. This message requires user touch interaction to display properly."

    const/4 v9, 0x1

    invoke-static {p1, p2}, Lcom/braze/support/BrazeLogger;->w(Ljava/lang/String;Ljava/lang/String;)I

    const/4 v9, 0x7

    goto :goto_0

    :cond_0
    const/4 v9, 0x4

    invoke-virtual {p1}, Landroid/app/Activity;->getApplicationContext()Landroid/content/Context;

    move-result-object p1

    move-object v1, p2

    move-object v1, p2

    const/4 v9, 0x1

    check-cast v1, Lcom/braze/models/inappmessage/InAppMessageHtmlFull;

    const/4 v9, 0x7

    new-instance v2, Le30;

    const/4 v9, 0x2

    invoke-direct {v2, p1, v1}, Le30;-><init>(Landroid/content/Context;Lcom/braze/models/inappmessage/IInAppMessageHtml;)V

    const/4 v9, 0x7

    invoke-interface {p2}, Lcom/braze/models/inappmessage/IInAppMessage;->getMessage()Ljava/lang/String;

    move-result-object v5

    const/4 v9, 0x4

    invoke-virtual {v1}, Lcom/braze/models/inappmessage/InAppMessageHtmlBase;->getLocalAssetsDirectoryUrl()Ljava/lang/String;

    move-result-object v1

    const/4 v9, 0x0

    invoke-virtual {v0}, Lh40;->getMessageWebView()Landroid/webkit/WebView;

    move-result-object v3

    const/4 v9, 0x7

    const-string v4, "l//mefi"

    const-string v4, "file://"

    const/4 v9, 0x0

    const-string v6, "/"

    const-string v6, "/"

    const/4 v9, 0x7

    invoke-static {v4, v1, v6}, Loy;->z0(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v4

    const/4 v9, 0x1

    const/4 v8, 0x0

    const/4 v9, 0x1

    const-string/jumbo v6, "x/ttoehml"

    const-string v6, "text/html"

    const/4 v9, 0x6

    const-string v7, "btu8f"

    const-string v7, "utf-8"

    const/4 v9, 0x3

    invoke-virtual/range {v3 .. v8}, Landroid/webkit/WebView;->loadDataWithBaseURL(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    const/4 v9, 0x7

    new-instance v1, Lx30;

    const/4 v9, 0x4

    iget-object v3, p0, Lz20;->a:Lo30;

    const/4 v9, 0x0

    invoke-direct {v1, p1, p2, v3}, Lx30;-><init>(Landroid/content/Context;Lcom/braze/models/inappmessage/IInAppMessage;Lo30;)V

    const/4 v9, 0x2

    invoke-virtual {v0, v1}, Lh40;->setInAppMessageWebViewClient(Lx30;)V

    const/4 v9, 0x5

    invoke-virtual {v0}, Lh40;->getMessageWebView()Landroid/webkit/WebView;

    move-result-object p1

    const/4 v9, 0x3

    const-string p2, "BregraunabIrntieedz"

    const-string p2, "brazeInternalBridge"

    const/4 v9, 0x4

    invoke-virtual {p1, v2, p2}, Landroid/webkit/WebView;->addJavascriptInterface(Ljava/lang/Object;Ljava/lang/String;)V

    move-object v2, v0

    move-object v2, v0

    :goto_0
    const/4 v9, 0x1

    return-object v2
.end method
