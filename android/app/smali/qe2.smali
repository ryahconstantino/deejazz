.class public final Lqe2;
.super Ljava/lang/Object;
.source ""

# interfaces
.implements Lzd2;


# annotations
.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000b\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0002\u0008\u0000\u0018\u00002\u00020\u0001BI\u0008\u0010\u0012\u0006\u0010\u0002\u001a\u00020\u0003\u0012\u0006\u0010\u0004\u001a\u00020\u0005\u0012\u0006\u0010\u0006\u001a\u00020\u0007\u0012\u0008\u0008\u0002\u0010\u0008\u001a\u00020\t\u0012\u0006\u0010\n\u001a\u00020\u000b\u0012\u0006\u0010\u000c\u001a\u00020\r\u0012\u0006\u0010\u000e\u001a\u00020\u000f\u0012\u0006\u0010\u0010\u001a\u00020\u0011\u00a2\u0006\u0002\u0010\u0012B=\u0008\u0001\u0012\u0012\u0010\u0013\u001a\u000e\u0012\u0004\u0012\u00020\u0015\u0012\u0004\u0012\u00020\u00160\u0014\u0012\u0006\u0010\n\u001a\u00020\u000b\u0012\u0018\u0010\u0017\u001a\u0014\u0012\u0004\u0012\u00020\u0019\u0012\u0004\u0012\u00020\u0016\u0012\u0004\u0012\u00020\u001a0\u0018\u00a2\u0006\u0002\u0010\u001bJ\u0010\u0010\u001c\u001a\u00020\u001d2\u0006\u0010\u001e\u001a\u00020\u0015H\u0016R\u000e\u0010\n\u001a\u00020\u000bX\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u001a\u0010\u0013\u001a\u000e\u0012\u0004\u0012\u00020\u0015\u0012\u0004\u0012\u00020\u00160\u0014X\u0082\u0004\u00a2\u0006\u0002\n\u0000R \u0010\u0017\u001a\u0014\u0012\u0004\u0012\u00020\u0019\u0012\u0004\u0012\u00020\u0016\u0012\u0004\u0012\u00020\u001a0\u0018X\u0082\u0004\u00a2\u0006\u0002\n\u0000\u00a8\u0006\u001f"
    }
    d2 = {
        "Lcom/deezer/core/auth/api/gateway/strategies/ArlStitch;",
        "Lcom/deezer/core/auth/api/gateway/UserAuthStitch;",
        "gatewayApi",
        "Lcom/deezer/core/gatewayapi/GatewayApi;",
        "okHttpClient",
        "Lokhttp3/OkHttpClient;",
        "gatewayAuthTokenProviderFactory",
        "Lcom/deezer/core/auth/api/gateway/GatewayAuthTokenProvider$Factory;",
        "objectMapper",
        "Lcom/fasterxml/jackson/databind/ObjectMapper;",
        "arlSignIn",
        "Lcom/deezer/core/auth/ArlSignIn;",
        "licenseDecryptor",
        "Lcom/deezer/core/auth/api/gateway/LicenseDecryptor;",
        "systemCurrentTimeProvider",
        "Lcom/deezer/core/commons/time/SystemCurrentTimeProvider;",
        "jsonResultBuilder",
        "Lcom/deezer/core/auth/api/gateway/GatewayJsonResultBuilder;",
        "(Lcom/deezer/core/gatewayapi/GatewayApi;Lokhttp3/OkHttpClient;Lcom/deezer/core/auth/api/gateway/GatewayAuthTokenProvider$Factory;Lcom/fasterxml/jackson/databind/ObjectMapper;Lcom/deezer/core/auth/ArlSignIn;Lcom/deezer/core/auth/api/gateway/LicenseDecryptor;Lcom/deezer/core/commons/time/SystemCurrentTimeProvider;Lcom/deezer/core/auth/api/gateway/GatewayJsonResultBuilder;)V",
        "authTokenProvider",
        "Lkotlin/Function1;",
        "Lcom/deezer/core/auth/ApiSession;",
        "Lcom/deezer/core/gatewayapi/UserInfoKeyProvider;",
        "call",
        "Lkotlin/Function2;",
        "Lcom/deezer/core/auth/api/gateway/GatewayUserAuthArlCallConfig;",
        "Lcom/deezer/core/auth/api/gateway/GatewayUserAuthArlCall;",
        "(Lkotlin/jvm/functions/Function1;Lcom/deezer/core/auth/ArlSignIn;Lkotlin/jvm/functions/Function2;)V",
        "proceed",
        "Lcom/deezer/core/auth/api/gateway/UserAuthResult;",
        "checkTokenApiSession",
        "core-lib__auth"
    }
    k = 0x1
    mv = {
        0x1,
        0x5,
        0x1
    }
    xi = 0x30
.end annotation


# instance fields
.field public final a:Ltpg;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ltpg<",
            "Lwa2;",
            "Ljv3;",
            ">;"
        }
    .end annotation
.end field

.field public final b:Lbb2;

.field public final c:Lxpg;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lxpg<",
            "Ltd2;",
            "Ljv3;",
            "Lsd2;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method public constructor <init>(Lsu3;Ll4i;Lgd2$a;Lcom/fasterxml/jackson/databind/ObjectMapper;Lbb2;Lwd2;Lno2;Lmd2;I)V
    .locals 12

    move-object v0, p0

    move-object v0, p0

    move-object v1, p3

    move-object v1, p3

    move-object/from16 v2, p5

    move-object/from16 v2, p5

    and-int/lit8 v3, p9, 0x8

    if-eqz v3, :cond_0

    sget-object v3, Ltb2;->a:Lcom/fasterxml/jackson/databind/ObjectMapper;

    const-string v4, "CBsRAOJ_PEMPT"

    const-string v4, "OBJECT_MAPPER"

    invoke-static {v3, v4}, Lrqg;->f(Ljava/lang/Object;Ljava/lang/String;)V

    goto :goto_0

    :cond_0
    const/4 v3, 0x0

    :goto_0
    move-object v6, v3

    move-object v6, v3

    const-string v3, "Aeimgpatwa"

    const-string v3, "gatewayApi"

    move-object v5, p1

    move-object v5, p1

    invoke-static {p1, v3}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v3, "poktottieCnH"

    const-string v3, "okHttpClient"

    move-object v7, p2

    move-object v7, p2

    invoke-static {p2, v3}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v3, "uyarebreriwnottchTtvFayoeAadkPg"

    const-string v3, "gatewayAuthTokenProviderFactory"

    invoke-static {p3, v3}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v3, "objectMapper"

    invoke-static {v6, v3}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v3, "rngSinuIl"

    const-string v3, "arlSignIn"

    invoke-static {v2, v3}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v4, "slcerytpncieproD"

    const-string v4, "licenseDecryptor"

    move-object/from16 v9, p6

    move-object/from16 v9, p6

    invoke-static {v9, v4}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v4, "uyrmnrorqtePiessemTiCdert"

    const-string v4, "systemCurrentTimeProvider"

    move-object/from16 v8, p7

    invoke-static {v8, v4}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v4, "dssBreutiusllRjeo"

    const-string v4, "jsonResultBuilder"

    move-object/from16 v10, p8

    move-object/from16 v10, p8

    invoke-static {v10, v4}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    new-instance v11, Loe2;

    invoke-direct {v11, p3}, Loe2;-><init>(Lgd2$a;)V

    new-instance v1, Lpe2;

    move-object v4, v1

    move-object v4, v1

    invoke-direct/range {v4 .. v10}, Lpe2;-><init>(Lsu3;Lcom/fasterxml/jackson/databind/ObjectMapper;Ll4i;Lno2;Lwd2;Lmd2;)V

    const-string v4, "eramkiorteTnohdPu"

    const-string v4, "authTokenProvider"

    invoke-static {v11, v4}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-static {v2, v3}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v3, "allc"

    const-string v3, "call"

    invoke-static {v1, v3}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object v11, v0, Lqe2;->a:Ltpg;

    iput-object v2, v0, Lqe2;->b:Lbb2;

    iput-object v1, v0, Lqe2;->c:Lxpg;

    return-void
.end method


# virtual methods
.method public a(Lwa2;)Lxd2;
    .locals 8
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/deezer/core/auth/AuthException;
        }
    .end annotation

    const-string v7, "  ~@~@~@~@~@~@~@~@~@~@~   Smob - Mod obfuscation tool v1.4 by Kirlif\'   ~@~@~@~@~@~@~@~@~@~@~  "

    const-string v0, "kcpsooikeescnoinAheT"

    const-string v0, "checkTokenApiSession"

    const/4 v7, 0x2

    invoke-static {p1, v0}, Lrqg;->g(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 v7, 0x2

    new-instance v0, Ltd2;

    const/4 v7, 0x4

    iget-object v1, p1, Lwa2;->b:Ljava/lang/String;

    const/4 v7, 0x7

    iget-object v2, p0, Lqe2;->b:Lbb2;

    const/4 v7, 0x4

    iget-object v3, v2, Lbb2;->a:Ljava/lang/String;

    const/4 v7, 0x2

    iget-object v2, v2, Lbb2;->b:Ljava/lang/String;

    const/4 v7, 0x5

    invoke-direct {v0, v1, v3, v2}, Ltd2;-><init>(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    const/4 v7, 0x7

    iget-object v1, p0, Lqe2;->c:Lxpg;

    const/4 v7, 0x4

    iget-object v2, p0, Lqe2;->a:Ltpg;

    const/4 v7, 0x0

    invoke-interface {v2, p1}, Ltpg;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object p1

    const/4 v7, 0x2

    invoke-interface {v1, v0, p1}, Lxpg;->invoke(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object p1

    const/4 v7, 0x2

    check-cast p1, Lsd2;

    const/4 v7, 0x1

    invoke-static {p1}, Ljava/util/Objects;->requireNonNull(Ljava/lang/Object;)Ljava/lang/Object;

    const/4 v7, 0x4

    const-string v0, "taearbwCGrlAhyeUAausll"

    const-string v0, "GatewayUserAuthArlCall"

    :try_start_0
    const/4 v7, 0x7

    const-string v1, "snAhuruu. ueRAs .rt )t.(eqieL"

    const-string v1, "requesting user Auth (ARL)..."

    const/4 v7, 0x1

    const/4 v2, 0x0

    const/4 v7, 0x6

    new-array v3, v2, [Ljava/lang/Object;

    const/4 v7, 0x4

    invoke-static {v0, v1, v3}, Lrb2;->a(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    const/4 v7, 0x1

    iget-object v1, p1, Lsd2;->d:Ll4i;

    const/4 v7, 0x3

    iget-object v3, p1, Lsd2;->a:Luc2;

    const/4 v7, 0x7

    iget-object v4, p1, Lsd2;->b:Ltd2;

    const/4 v7, 0x0

    iget-object v5, v4, Ltd2;->a:Ljava/lang/String;

    const/4 v7, 0x3

    iget-object v6, v4, Ltd2;->b:Ljava/lang/String;

    const/4 v7, 0x6

    iget-object v4, v4, Ltd2;->c:Ljava/lang/String;

    const/4 v7, 0x2

    invoke-interface {v3, v5, v6, v4}, Luc2;->p(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ln4i;

    move-result-object v3

    const/4 v7, 0x1

    invoke-virtual {v1, v3}, Ll4i;->a(Ln4i;)Lm3i;

    move-result-object v1
    :try_end_0
    .catch Lcom/deezer/core/auth/AuthException; {:try_start_0 .. :try_end_0} :catch_2
    .catch Lcom/deezer/core/gatewayapi/error/ApiException; {:try_start_0 .. :try_end_0} :catch_1
    .catch Ljava/lang/Exception; {:try_start_0 .. :try_end_0} :catch_0

    const/4 v7, 0x2

    check-cast v1, Lt5i;

    :try_start_1
    const/4 v7, 0x2

    invoke-virtual {v1}, Lt5i;->execute()Ls4i;

    move-result-object v1
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    const/4 v7, 0x7

    goto :goto_0

    :catchall_0
    move-exception v1

    :try_start_2
    const/4 v7, 0x3

    invoke-static {v1}, Lxkg;->h0(Ljava/lang/Throwable;)Ljava/lang/Object;

    move-result-object v1

    :goto_0
    const/4 v7, 0x2

    invoke-static {v1}, Ldmg;->a(Ljava/lang/Object;)Ljava/lang/Throwable;

    move-result-object v3

    const/4 v7, 0x7

    if-eqz v3, :cond_0

    const/4 v7, 0x5

    invoke-static {v3}, Lcom/deezer/core/auth/AuthException;->b(Ljava/lang/Throwable;)Lcom/deezer/core/auth/AuthException;

    :cond_0
    const/4 v7, 0x6

    invoke-static {v1}, Lxkg;->j4(Ljava/lang/Object;)V

    const/4 v7, 0x4

    check-cast v1, Ls4i;

    invoke-virtual {v1}, Ls4i;->d()Z

    move-result v3

    const/4 v7, 0x2

    if-eqz v3, :cond_3

    const/4 v7, 0x4

    iget-object v1, v1, Ls4i;->h:Lt4i;

    const/4 v7, 0x6

    if-eqz v1, :cond_2

    const/4 v7, 0x0

    iget-object v3, p1, Lsd2;->c:Lcom/fasterxml/jackson/databind/ObjectMapper;

    const/4 v7, 0x5

    invoke-virtual {v1}, Lt4i;->h()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v3, v1}, Lcom/fasterxml/jackson/databind/ObjectMapper;->readTree(Ljava/lang/String;)Lcom/fasterxml/jackson/databind/JsonNode;

    move-result-object v1

    const/4 v7, 0x0

    new-instance v3, Lyv3;

    const/4 v7, 0x3

    invoke-direct {v3}, Lyv3;-><init>()V

    const/4 v7, 0x5

    const-string v4, "rTosnejp"

    const-string v4, "jsonTree"

    const/4 v7, 0x0

    invoke-static {v1, v4}, Lrqg;->f(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 v7, 0x7

    iget-object v4, p1, Lsd2;->c:Lcom/fasterxml/jackson/databind/ObjectMapper;

    invoke-static {v1, v4}, Lig2;->e(Lcom/fasterxml/jackson/databind/JsonNode;Lcom/fasterxml/jackson/core/ObjectCodec;)Lcom/fasterxml/jackson/core/JsonParser;

    move-result-object v4

    const/4 v7, 0x4

    const-string v5, "jsonTree.traverseAndInit(objectMapper)"

    const/4 v7, 0x4

    invoke-static {v4, v5}, Lrqg;->f(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 v7, 0x4

    invoke-virtual {v3, v4}, Lyv3;->b(Lcom/fasterxml/jackson/core/JsonParser;)V

    const/4 v7, 0x6

    const-string v3, "lqserut"

    const-string v3, "results"

    const/4 v7, 0x1

    invoke-virtual {v1, v3}, Lcom/fasterxml/jackson/databind/JsonNode;->get(Ljava/lang/String;)Lcom/fasterxml/jackson/databind/JsonNode;

    move-result-object v3

    const/4 v7, 0x1

    if-eqz v3, :cond_1

    const/4 v7, 0x1

    const-string v1, "cos see njidvRse"

    const-string v1, "Received json %s"

    const/4 v7, 0x4

    const/4 v4, 0x1

    const/4 v7, 0x6

    new-array v4, v4, [Ljava/lang/Object;

    const/4 v7, 0x5

    aput-object v3, v4, v2

    const/4 v7, 0x5

    invoke-static {v0, v1, v4}, Lrb2;->a(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    const/4 v7, 0x4

    iget-object v0, p1, Lsd2;->c:Lcom/fasterxml/jackson/databind/ObjectMapper;

    const/4 v7, 0x6

    invoke-static {v3, v0}, Lig2;->f(Lcom/fasterxml/jackson/databind/JsonNode;Lcom/fasterxml/jackson/core/ObjectCodec;)Lcom/fasterxml/jackson/core/JsonParser;

    move-result-object v0

    const/4 v7, 0x3

    new-instance v1, Lyd2;

    const/4 v7, 0x4

    iget-object v2, p1, Lsd2;->g:Lmd2;

    const/4 v7, 0x2

    iget-object v3, p1, Lsd2;->e:Lod2;

    const/4 v7, 0x0

    iget-object v4, p1, Lsd2;->f:Lwd2;

    const/4 v7, 0x2

    iget-object p1, p1, Lsd2;->a:Luc2;

    const/4 v7, 0x6

    invoke-interface {p1}, Lsu3;->M()Lyu3;

    move-result-object p1

    const/4 v7, 0x4

    iget-object p1, p1, Lyu3;->b:Lav3;

    const/4 v7, 0x5

    invoke-interface {p1}, Lzu3;->e()Ljava/lang/String;

    move-result-object p1

    const/4 v7, 0x4

    invoke-direct {v1, v2, v3, v4, p1}, Lyd2;-><init>(Lmd2;Lod2;Lwd2;Ljava/lang/String;)V

    const/4 v7, 0x0

    const-string p1, "sremar"

    const-string p1, "parser"

    const/4 v7, 0x6

    invoke-static {v0, p1}, Lrqg;->f(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 v7, 0x3

    invoke-virtual {v1, v0}, Lyd2;->a(Lcom/fasterxml/jackson/core/JsonParser;)Lxd2;

    move-result-object p1

    const/4 v7, 0x1

    return-object p1

    :cond_1
    const/4 v7, 0x6

    const-string p1, "n t ounncno fntd oiicca"

    const-string p1, "cannot find account in "

    const/4 v7, 0x6

    invoke-static {p1, v1}, Lrqg;->l(Ljava/lang/String;Ljava/lang/Object;)Ljava/lang/String;

    move-result-object p1

    const/4 v7, 0x5

    invoke-static {p1}, Lcom/deezer/core/auth/AuthException;->d(Ljava/lang/String;)Lcom/deezer/core/auth/AuthException;

    move-result-object p1

    const/4 v7, 0x5

    throw p1

    :cond_2
    const/4 v7, 0x1

    const-string p1, "  otAbeasdUodbil avrre fcteaell uyd),ull (h clir e"

    const-string p1, "call to User Auth (arl) failed, received null body"

    const/4 v7, 0x6

    invoke-static {p1}, Lcom/deezer/core/auth/AuthException;->d(Ljava/lang/String;)Lcom/deezer/core/auth/AuthException;

    move-result-object p1

    const/4 v7, 0x3

    throw p1

    :cond_3
    const/4 v7, 0x4

    iget p1, v1, Ls4i;->e:I

    const/4 v7, 0x5

    new-instance v0, Lcom/deezer/core/auth/AuthException$AuthHttpStatusCodeException;

    const/4 v7, 0x6

    invoke-direct {v0, p1}, Lcom/deezer/core/auth/AuthException$AuthHttpStatusCodeException;-><init>(I)V

    const/4 v7, 0x4

    throw v0
    :try_end_2
    .catch Lcom/deezer/core/auth/AuthException; {:try_start_2 .. :try_end_2} :catch_2
    .catch Lcom/deezer/core/gatewayapi/error/ApiException; {:try_start_2 .. :try_end_2} :catch_1
    .catch Ljava/lang/Exception; {:try_start_2 .. :try_end_2} :catch_0

    :catch_0
    move-exception p1

    const/4 v7, 0x3

    invoke-static {p1}, Lcom/deezer/core/auth/AuthException;->c(Ljava/lang/Throwable;)Lcom/deezer/core/auth/AuthException;

    move-result-object p1

    const/4 v7, 0x7

    throw p1

    :catch_1
    move-exception p1

    const/4 v7, 0x1

    invoke-static {p1}, Lcom/deezer/core/auth/AuthException;->a(Lcom/deezer/core/gatewayapi/error/ApiException;)Lcom/deezer/core/auth/AuthException;

    move-result-object p1

    const/4 v7, 0x3

    throw p1

    :catch_2
    move-exception p1

    const/4 v7, 0x0

    throw p1
.end method
