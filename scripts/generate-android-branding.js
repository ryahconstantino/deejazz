...
async function main() {
  ...
  if (glyphs.length !== 7 || /<(text|font)\b/.test(wordmark)) {
...
<bitmap ${namespace} android:src="@drawable/deejazz_launcher_art" android:gravity="fill" android:filter="true"/>\n`);
    ...
    } else if (name === "ic_deezer_logo_colored_no_wording.xml" || /^icon_deezer_logo_/.test(name)) {
      // Shared colorful symbol, without stacked lettering.
      ...
      const content = paths(glyphs, color);
      ...
      const source = name === "launcher_ic_app.png" || name === "ic_deezer_logo_colored.png" ||
        ...
        /widget_logo/.test(name) ? launcher :
  ...
  for (const file of stringFiles) {
    ...
    let branded = original.split(/(<[^>]+>)/g).map(part => part.startsWith("<") ? part : part
      ...
      .replace(/(<string name="dz_copyright_title_copyright_mobile">[^<]*)DeeJazz\.com/gi, "$1DeeJazz")
      .replace(/[ \t]+$/gm, "");
